...
from term_structures.curvebond import CurveBond
...
class BootstrappedCurve(Curve):
    ...
    """
    def __init__(self):
        self.instruments = {}  # Map each maturity to an instrument
        super().__init__([], [])

    def add_instrument(self, bond):
        """  Save instrument info by maturity """
        self.instruments[bond.maturity_term] = bond
    ...
    def factors(self):
        ...
        for bond in self.instruments.values():
            self.instruments[bond.maturity_term].spot_rate = \
                self.bond_spot_rate(bond)

        return [self.instruments[t].spot_rate for t in self.times]

    def bond_spot_rate(self, bond: CurveBond) -> float:
        ...
        if bond.coupon == 0:
            ...
            for i in range(1, number_of_periods):
                ...
                rate = self.instruments[term].spot_rate
...
class TestBootstrap(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.yield_curve.add_instrument(CurveBond(100, 0.25, 0., 97.5))
        self.yield_curve.add_instrument(CurveBond(100, 0.5, 0., 94.9))
        self.yield_curve.add_instrument(CurveBond(100, 1.0, 0., 90.))
        self.yield_curve.add_instrument(CurveBond(100, 1.5, 8.0, 96., 2))
        self.yield_curve.add_instrument(CurveBond(100, 2., 12.0, 101.6, 2))
    ...
    def test_bootstrap(self):
        """test bootstrap"""
        rates = []
        rates.append(0.10127123193715915)
        rates.append(0.10469296074441839)
        rates.append(0.10536051565782635)
        rates.append(0.1068092638817053)
        rates.append(0.10808027549746793)
        self.assertEqual(self.yield_curve.factors, rates)

    def test_interpolator(self):
        """test interpolation"""
        for term in range(4):
            self.assertEqual(self.yield_curve(self.yield_curve.times[term]),
                             self.yield_curve.factors[term])
